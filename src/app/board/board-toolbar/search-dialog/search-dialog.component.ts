...
export class SearchDialogComponent implements OnChanges {
  ...
  idxToFindPositionOfClickedChatRoom!: number;
  ...
  async showTheClickedElementOfTypePrivatChat(clickedElement: PrivateChat, event: Event) {
    this.idxToFindPositionOfClickedChatRoom = this.findIdIndexInDirectMessages(clickedElement);
    this.selectChatRoomAndMember(event)
  }

  findIdIndexInDirectMessages(clickedElement: PrivateChat): number {
    return this.firestore.directMessages.findIndex(privChat => privChat.id == clickedElement.id);
  }

  async selectChatRoomAndMember(event: Event): Promise<void> {
    this.boardServ.selectedChatRoom = this.firestore.directMessages[this.idxToFindPositionOfClickedChatRoom];
    if (this.boardServ.currentUser.id == this.boardServ.selectedChatRoom.creator.id) {
      this.memberServ.currentMember = this.boardServ.selectedChatRoom.guest;
      await this.boardServ.startPrivateChat(this.idxToFindPositionOfClickedChatRoom, 'creator', event);
      await this.scrollToMessageAfterPrivatChatSetted();
    } else {
      this.memberServ.currentMember = this.boardServ.selectedChatRoom.creator;
      this.boardServ.startPrivateChat(this.idxToFindPositionOfClickedChatRoom, 'guest', event);
      await this.scrollToMessageAfterPrivatChatSetted();
    }
  }

  async scrollToMessageAfterPrivatChatSetted(): Promise<void> {
  ...
  findMessageIndexInSelectedChatRoom(): number {
    return this.boardServ.selectedChatRoom.chat.findIndex((chat) => chat.message.trim().toLowerCase().includes(this.searchValue.toLowerCase()))
  }
